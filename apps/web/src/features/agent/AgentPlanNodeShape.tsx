...
const labels: Record<Locale, {
  ...
  "zh-TW": {
    ...
    badPlanCopy: "快照中的計劃節點資料已損壞，已安全保留節點但不執行任何操作。",
    badPlanTitle: "無法讀取 Agent 計劃",
    count: (count) => `${count} 張`,
    countLabel: "數量",
    dependencies: "依賴",
    dependencyChip: (jobId) => `依賴 ${jobId}`,
    detailTitle: "任務詳情",
    emptySlot: "等待縮圖",
    emptyOutputSummary: "暫無輸出圖",
    errorFallback: "任務失敗，暫無更多錯誤資訊。",
    errorLabel: "錯誤",
    finalOutputCount: (count) => `${count} 張最終圖`,
    generatedReference: (jobId) => `來自 ${jobId}`,
    jobs: "任務",
    noDependencies: "無依賴",
    noOutputs: "沒有輸出槽",
    noReferences: "無引用",
    outputFailed: "輸出失敗",
    outputCount: (count) => `預計 ${count} 張圖`,
    outputReady: "縮圖已就緒",
    outputSlot: (index) => `輸出 ${index}`,
    outputs: "輸出槽",
    prompt: "完整提示詞",
    references: "參考",
    roleLabel: "角色",
    ...
    supportOutputCount: (count) => `${count} 張支援圖`,
    jobStatus: {
      queued: "排隊",
      running: "生成中",
      succeeded: "完成",
